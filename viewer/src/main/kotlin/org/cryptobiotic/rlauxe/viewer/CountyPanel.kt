...
class CountyPanel(
    ...
    init {
        ...
        contestTable = BeanTable<CountyContestBean>(
        ...
        contestTable.addPopupOption(
            "Show Contest", contestTable.makeShowAction(
                infoTA, infoWindow,
                Function { bean: Any? -> showContest((bean as ContestsPanel.ContestBean?)!!) })
        )
    ...
    override fun setAuditRecord(auditRecordLocation: String): Boolean {
        ...
        try {
            ...
            val npop: Int = (if (statewide == null) 0 else statewide.npop)
            val totalBean = CountyBean( CountyData("=Total", countUniformMvrs, npop))
        ...
        for (countyContest in countyContestData) {
            ...
            if (contestRound != null && contestRound.status == TestH0Status.InProgress) {
                if (countyContest.countyName == countyBean.name) {
                    ...
                    beanList.add(bean)
                    bean.countyPopulation = countyBean.npop
                    bean.nmvrsConsistent = countyBean.nmvrsConsistent
                    bean.nmvrsUniform = countyBean.nmvrsUniform
    ...
    fun showContest(contestBean: ContestsPanel.ContestBean) = buildString {
    ...
    companion object {
        private val logger: Logger = LoggerFactory.getLogger(CountyPanel::class.java)
    }
}
...
class CountyContestBean(val countyContestData: CountyContestData, val auditRiskLimit: Double) {
    ...
    fun getPopSize() = contestUA?.population() ?: 1

    fun getMargin() = countyContestData.voteDiff / getPopSize().toDouble()
    fun getNoerror() = noerror(getMargin(), 1.0) // TODO upper
    ...
    fun getEstRisk(): String {
        ...
        return dfn(estRiskStandardBet(getPopSize(), getNoerror(), haveMvrs), 4)
    }

    fun getEstMvrs(): String {
        if (!contained()) return ""
        return nfn(estSampleSizeStandardBet(getPopSize(), getNoerror(), maxrisk()), -6)
    }
