...
public class BAMutil {
  ...
  private static JMenuItem makeMenuItem(Icon icon, Icon rollover, String menu_cmd, boolean is_toggle, int mnemonic,
      int accel) {
    JMenuItem mi;
    if (is_toggle)
   ...
   * are optional except for Action.SHORT_DESCRIPTION:
   * 
   * <pre>
   ...
   * and MenuItem state changes when the Action state changes. <br>
   * <br>
  ...
  public static JMenuItem addActionToMenu(JMenu menu, Action act, int menuPos) {
    ...
    if (is_toggle) {
      if (debugToggle)
        System.out
            .println("addActionToMenu: " + act.getValue(Action.SHORT_DESCRIPTION) + " " + act.getValue(BAMutil.STATE));
  ...
  private static AbstractButton _makeButtconFromAction(Action act) {
    ...
    return makeButtcon((Icon) act.getValue(Action.SMALL_ICON), (Icon) act.getValue(BAMutil.SELECTED_ICON),
        (String) act.getValue(Action.SHORT_DESCRIPTION), is_toggle);
  }

  /**
   * creates an AbstractButton using the given Action and adds it to the given Container at the position..
   * Uses Properties that have been set on the Action (see setActionProperties()). All
   * are optional except for Action.SMALL_ICON:
   * 
   * <pre>
   ...
   * and the Button state changes when the Action state changes. <br>
   * <br>
   * The point of all this is that once you set it up, you work exclusively with the action object,
   * and all changes are automatically reflected in the UI.
   * 
  ...
  public static AbstractButton addActionToContainerPos(Container c, Action act, int pos) {
    ...
    if (is_toggle) {
      if (debugToggle)
        System.out.println(
            "addActionToContainerPos: " + act.getValue(Action.SHORT_DESCRIPTION) + " " + act.getValue(BAMutil.STATE));
      Boolean state = (Boolean) act.getValue(BAMutil.STATE);
      if (state == null)
        state = Boolean.FALSE;
      act.putValue(BAMutil.STATE, state);
  ...
  private static class ToggleAction extends AbstractAction {
    ...
    public void actionPerformed(java.awt.event.ActionEvent e) {
      ...
      if (debugToggle)
        System.out.println(
            "ToggleAction: " + orgAct.getValue(Action.SHORT_DESCRIPTION) + " " + orgAct.getValue(BAMutil.STATE));
      orgAct.actionPerformed(e);
    }
  }
